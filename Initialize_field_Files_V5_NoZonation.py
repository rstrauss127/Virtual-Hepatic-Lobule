...
def Initialize_Zonation_File():
    ...
    valueGSHInitCentral = 0.7e-14 ## mol
    valueGSHInitPortal = 0.7e-14 ## mol
    ## Less production in central   Initial value of the model = 1.375e-14
    # percentVariation = 0.2
    valueGSHProcducRateCentral = 1.375e-14/8.64e4# - (1.175e-14/8.64e4 * percentVariation) ## mol
    valueGSHProductRatePortal = 1.375e-14/8.64e4# + (1.175e-14/8.64e4 * percentVariation) ## mol
    ## Less binding in portal       Initial value of the model = 1.6e18
    # percentVariation = 0.2
    valueGSHBindingRateCentral = 1.6e18/8.64e4#  + (1.6e18/8.64e4 * percentVariation) ## mol
    valueGSHBindingRatePortal = 1.6e18/8.64e4# - (1.6e18/8.64e4 * percentVariation) ## mol
    ## More Regen in Portal         Initial value of the model = 1
    # percentVariation = 0.2
    valueRegenRateCentral = 1/8.64e4# - (1/8.64e4 * percentVariation) ## mol
    valueRegenRatePortal = 1/8.64e4# + (1/8.64e4 * percentVariation)## mol 
    ...
    for line in Nodes:
        ...
        GSHInitValue = a * np.sqrt(float(nodeCoords[1]) * float(nodeCoords[1]) + float(nodeCoords[2]) * float(nodeCoords[2])) + b
        GSHProducRateValue = c * np.sqrt(float(nodeCoords[1]) * float(nodeCoords[1]) + float(nodeCoords[2]) * float(nodeCoords[2])) + d
        GSHBindingRateValue = e * np.sqrt(float(nodeCoords[1]) * float(nodeCoords[1]) + float(nodeCoords[2]) * float(nodeCoords[2])) + f
        regenRateValue = g * np.sqrt(float(nodeCoords[1]) * float(nodeCoords[1]) + float(nodeCoords[2]) * float(nodeCoords[2])) + h
        random = rd.random()
